  ...
  return <main id="top">
    ...
    <article id="essay">
      ...
      <figure className="galois-figure wide" aria-labelledby="galois-figure-title">
        <div className="diagram-heading"><h3 id="galois-figure-title">Some swaps work. Others break the arithmetic.</h3><p>Here is the same test for x⁴ = 2. We use α for the positive number whose fourth power is 2. The symbol i is the imaginary unit, defined by i² = −1. There are four roots: α, −α, iα, and −iα. The diagram places the real roots horizontally and the imaginary roots vertically.</p></div>
      ...
      <Section id="hill-climbing" title="What were we actually climbing?" note={<><p>Propose a change.<br/>Evaluate it.<br/>Keep improvements.<br/>Try again.</p></>}>
        ...
        <p>Andrej Karpathy helped popularise the term <strong>autoresearch</strong> with a small, concrete example. An AI agent changes the code used to train a language model, runs a five-minute training experiment, and checks how well the resulting model predicts text it was not trained on. It retains changes that improve that result, reverts unsuccessful ones, and repeats.<Ref n={10}/> The human sets the instructions, the time budget, and the test. The agent can then carry out many experiments without waiting for a person between each one.</p>
        <p>Autoresearch lets an agent run that cycle of experiments and feedback. <strong>Hill climbing is one way to decide which changes to keep.</strong> The familiar picture is below: each position represents a possible candidate, and height represents how well it scores. Moving uphill gets you to something better nearby. It can also leave you on top of a small hill, with a higher one across a valley. A fresh starting point or a different kind of move can help the search explore elsewhere. Our persistent runs had this rhythm, with Durgesh and me still choosing mathematical directions and reviewing what the experiments meant.</p>
      </Section>
      <figure className="hill-figure wide" aria-labelledby="hill-figure-title">
        <div className="diagram-heading"><h3 id="hill-figure-title">Getting uphill is easier than finding the highest hill.</h3><p>Keep a change when it improves the result. Repeat until nearby changes stop helping.</p></div>
        <picture><source media="(max-width:760px)" srcSet="/figures/hill-climbing-mobile.svg"/><img src="/figures/hill-climbing.svg" alt="A schematic search landscape. Blue steps climb from a starting point to a local peak. Beyond a valley lies a higher, global peak; farther right is a flat plateau. Ordinary uphill moves can stop at the local peak even though the global peak is better." width="1100" height="530" loading="lazy"/></picture>
        <figcaption>A local maximum is better than its neighbors. The global maximum is the best point across the whole landscape. A plateau offers little guidance because nearby choices score the same. This is a schematic of the search idea, not a plot of our IGP24 results. Our constructions form a much more complicated space, and the competition’s scoring landscape also changes as other teams submit.<Ref n={7}/></figcaption>
      </figure>
    ...
    <footer className="appendix" aria-labelledby="appendix-title">
      ...
      <div><ol className="sources">
        ...
        <li id="source-10">Andrej Karpathy’s <a href="https://github.com/karpathy/autoresearch">autoresearch</a>: the March 2026 project and its <a href="https://github.com/karpathy/autoresearch/blob/master/program.md">experiment instructions</a>. The comparison with our IGP24 workflow is ours.</li>
