#!/usr/bin/env python3
"""Prepare immutable profile-backfill batch 2 on batch-1 exact coverage."""
...
ROOT = planner.ROOT
DEST = planner.DATA / "gold_profile_backfill_20260722_batch2"
...
def configure() -> None:
    ...
    planner.BATCH_NUMBER = 2
    planner.ALLOWED_FULL_STATUSES = {"certified_fresh_gold_candidates_found"}
    planner.REQUIRE_ZERO_FRONTIER_CERTIFICATE = False
    planner.REQUIRE_FULL_VOLATILE_BOUNDARY_MATCH = False
    planner.REQUIRE_FULL_LEDGER_BOUNDARY_MATCH = False
    planner.PRIORITIZE_CONDITIONAL_FRONTIER = False
    planner.EXCLUDED_PROFILE_PLAN = (
        planner.DATA / "gold_profile_backfill_20260722_batch1/provenance_plan.json"
    )
    planner.REQUIRED_PROFILE_COVERAGE = (
        planner.DATA / "gold_profile_backfill_20260722_batch1/missing_profile_rows.jsonl",
    )
