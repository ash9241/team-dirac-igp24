#!/usr/bin/env python3
"""Prepare max-throughput immutable batch 7 on exact batch1-6/v21 coverage."""
...
ROOT = planner.ROOT
DEST = planner.DATA / "gold_profile_backfill_20260722_batch7"
EXPECTED_FULL_AUDIT_SHA256 = (
    "527848db1ab4496de4a35363db1c00357184b0970083b9b50e87a3b31402e171"
)
EXPECTED_BOUNDARY = {
    "acceptedScoreablePairs": 22_062,
    "acceptedScoreableAnchorRows": 927_813,
    "acceptedPairSetSha256": "3cbfb63b7b1e18be2873be0b9a3be937d4b6bf36727705261dd1a8e487624dbf",
    "targetRows": 165_836,
    "targetSnapshotSha256": "7ee2fed7ee72aebfe0f0f17c8ea3d2a3833f48b244597e353d55eba0ea1717f6",
    "receiptAndOutboxBoundary": {
        "receipts": {
            "files": 136,
            "indexSha256": "ea3981cb3f2cd6204eced146beb9ec20278cce43aca9bb545d1c3f289747440e",
        },
        "outboxes": {
            "files": 402,
            "indexSha256": "4117ff79d7d3551e88cf47c573f72fe364be7cd02c62ba40fae102f38ade1697",
        },
    },
}
...
def validate_frozen_boundary() -> None:
    ...
    if planner.helper.sha256_path(planner.audit.CERTIFICATE) != EXPECTED_FULL_AUDIT_SHA256:
        raise ValueError("batch7 full-ledger audit certificate changed")
    if any(boundary.get(key) != value for key, value in EXPECTED_BOUNDARY.items()):
        raise ValueError("batch7 exact accepted/target/receipt/outbox boundary changed")
...
def configure() -> None:
    ...
    planner.REQUIRED_ACTION_AUDIT_FLAGS = tuple(
        f"profileBackfillBatch{number}OutputComplete" for number in range(1, 7)
    ) + ("v21OutputComplete",)
