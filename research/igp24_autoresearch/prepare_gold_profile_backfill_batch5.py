#!/usr/bin/env python3
"""Prepare immutable profile-backfill batch 5 on exact batch1-4/v21 coverage."""
...
ROOT = planner.ROOT
DEST = planner.DATA / "gold_profile_backfill_20260722_batch5"

EXPECTED_FULL_AUDIT_SHA256 = (
    "9223924fc4694d9be1437dbba781104078bae8fb7cb373838f505e74c736ed5d"
)
EXPECTED_BOUNDARY = {
...
def validate_frozen_boundary() -> None:
    ...
    if planner.helper.sha256_path(planner.audit.CERTIFICATE) != EXPECTED_FULL_AUDIT_SHA256:
        raise ValueError("batch5 full-ledger audit certificate changed")
    if any(boundary.get(key) != value for key, value in EXPECTED_BOUNDARY.items()):
        raise ValueError("batch5 exact accepted/target/receipt/outbox boundary changed")
...
def configure() -> None:
    ...
    planner.REQUIRED_ACTION_AUDIT_FLAGS = tuple(
        f"profileBackfillBatch{number}OutputComplete" for number in (1, 2, 3, 4)
    ) + ("v21OutputComplete",)
