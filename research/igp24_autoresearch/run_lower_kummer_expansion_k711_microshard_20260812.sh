...
shard="$1"
cd /path/to/igp24
summary="data/current_lower_kummer_expansion_k711_summary_micro${shard}_of256_20260812.json"
if [[ -f "$summary" ]]; then
  exit 0
fi
...
env SAGE_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 sage -python \
  ...
  --actions "data/current_lower_kummer_expansion_k711_actions_micro${shard}_of256_20260812.jsonl" \
  --routes "data/current_lower_kummer_expansion_k711_routes_micro${shard}_of256_20260812.jsonl" \
  --summary "$summary" \
  --subset-sizes 7,8,9,10,11 \
  > "k711_micro${shard}.log" 2>&1
