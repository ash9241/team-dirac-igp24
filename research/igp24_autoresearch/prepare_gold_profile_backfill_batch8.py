#!/usr/bin/env python3
"""Prepare final max-50 immutable batch 8 on exact batch1-7/v21 coverage."""
...
DEST = planner.DATA / "gold_profile_backfill_20260722_batch8"
EXPECTED_FULL_AUDIT_SHA256 = (
    "f9632c603aa71cf4e45c44727096f1972c5423aca8b4c58caea62897413fe22f"
)
EXPECTED_BOUNDARY = {
...
def validate_frozen_boundary() -> None:
    ...
    if planner.helper.sha256_path(planner.audit.CERTIFICATE) != EXPECTED_FULL_AUDIT_SHA256:
        raise ValueError("batch8 full-ledger audit certificate changed")
    if any(boundary.get(key) != value for key, value in EXPECTED_BOUNDARY.items()):
        raise ValueError("batch8 exact accepted/target/receipt/outbox boundary changed")
...
def configure() -> None:
    ...
    planner.REQUIRED_ACTION_AUDIT_FLAGS = tuple(
        f"profileBackfillBatch{number}OutputComplete" for number in range(1, 8)
    ) + ("v21OutputComplete",)
