...
"""Run q214 exact lift using its accepted parent's unique block quotient."""
...
QUOTIENT_T = 214
QUOTIENT_ORDER = 1296
SOURCE_LABEL = "24T23278"
BLOCK_KERNEL_ORDER = 4096


def load_driver():
    spec = importlib.util.spec_from_file_location("q214_fast_broad", BASE_PATH)
...
def structural_certificate(field_row: dict, quotient_t: int, _ring) -> dict:
    if int(quotient_t) != QUOTIENT_T:
        raise ValueError("q214 fast launcher received another quotient")
    source_rows = field_row.get("sourceRows", [])
    if not source_rows or {
        str(row.get("label")) for row in source_rows
    } != {SOURCE_LABEL}:
        raise ValueError("q214 source-label pin changed")
    if not all(row.get("scoreable") is True for row in source_rows):
        raise ValueError("q214 source row is not accepted/scoreable")
    ...
    with STRUCTURES.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if row.get("label") == SOURCE_LABEL:
                parents.append(row)
    if len(parents) != 1:
        raise ValueError("q214 structural parent is absent or duplicated")
    systems = [
        system
    ...
    if len(systems) != 1:
        raise ValueError("q214 parent no longer has a unique 12x2 system")
    system = systems[0]
    if (
        ...
        raise ValueError("q214 structural quotient certificate changed")
    return {
        "algorithm": (
