#!/usr/bin/env python3
"""Rebase the sealed tc9 slice onto current receipts, outboxes, and targets."""
...
OUTPUT = ROOT / "data/low_contention_tc9_current_routes_certificate.json"
BATCH_NAME = "low_contention_tc9_20260722"
NAMED_RECEIPTS = {
...
def main() -> int:
    ...
    selected = copy.deepcopy(routes[20:30])
    if [int(route.get("layerRank", -1)) for route in selected] != list(range(1, 11)):
        raise coordinator.GuardFailure("tc9 layer order changed")
    ...
    with closing(coordinator.connect_ro(config)) as connection:
        ...
        for route in selected:
            ...
            if current is None or int(current["discovered"] or 0) != 1 or (
                str(current["minimum_disc_abs"])
                != str(target["minimumDiscAbsAtSeal"])
            ):
                raise coordinator.GuardFailure(
                    f"tc9 target score state changed: {coordinator.pair_text(pair)}"
                )
            if int(current["team_count"]) == 9 and not owned:
                pass
            elif pair == ("24T18866", 4) and int(current["team_count"]) == 10 and owned:
                known_stale_pairs.append({
                    "targetPair": coordinator.pair_text(pair),
                    "sealedTeamCount": 9,
                    "currentTeamCount": 10,
                    "locallyOwned": True,
                    "decision": "retain_route_record_and_fail_closed_before_worker",
                })
            else:
                raise coordinator.GuardFailure(
                    f"unexpected tc9 target ownership/team change: {coordinator.pair_text(pair)}"
                )
            ...
            target_refresh.append({
                ...
                "teamCount": int(current["team_count"]),
                "discovered": True,
                "locallyOwned": owned,
    ...
    with closing(coordinator.connect_ro(config)) as connection:
        for route in selected:
            pair = coordinator.parse_pair(route["target"])
            try:
                guard = coordinator.guard_route(connection, route, exclusions)
                route_audits.append({
                    "routeId": route["routeId"],
                    "targetPair": coordinator.pair_text(pair),
                    "status": "ready_current_tc9_novel",
                    "guard": guard,
                })
            except coordinator.GuardFailure as exc:
                if pair != ("24T18866", 4):
                    raise
                route_audits.append({
                    "routeId": route["routeId"],
                    "targetPair": coordinator.pair_text(pair),
                    "status": "known_stale_fail_closed_before_worker",
                    "reason": coordinator.redact(str(exc)),
                })
    named = [
    ...
    value = {
        "schemaVersion": "low-contention-current-tc9-frontier-v1",
        "createdAt": coordinator.now(),
        "status": "certified_current_receipt_outbox_aware_tc9_runbooks_ready",
        "scope": "current_boundary_tc9_only_ten_ranked_routes",
        ...
        "knownStalePairAudit": known_stale_pairs,
        "routeAudits": route_audits,
        "checks": {
            "parentCertificateValidated": True,
            "selectedTenRoutesAreExactlyTc9": True,
            "tc7Tc8RoutesExcludedFromDerivative": True,
            "currentBoundaryCaptured": True,
            "nineTargetsRemainCurrentTc9Novel": True,
            "oneOwnedTc10TargetPinnedForFailClosedSkip": True,
            "targetDiscoveryAndMinimumDiscUnchanged": True,
            ...
            "allTenSourcesAcceptedScoreable": True,
            "allExecutableTargetsCurrentNovel": True,
