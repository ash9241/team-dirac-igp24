...
import seal_low_contention_tc3_frontier as seal


class SealLowContentionTc3FrontierTests(unittest.TestCase):
    def test_two_receipts_are_intact_one_plus_twenty_two(self) -> None:
        receipts, by_hash = seal.load_receipts()
        self.assertEqual(receipts[seal.FIRST_RECEIPT]["manifest"]["polynomials"], 1)
        self.assertEqual(receipts[seal.TAIL_RECEIPT]["manifest"]["polynomials"], 22)
        self.assertEqual(len(by_hash), 23)

    def test_route_frontier_has_twenty_three_complete_chains(self) -> None:
        certificate = lane.read_json(seal.ROUTE_CERTIFICATE)
        self.assertEqual(len(certificate["runbooks"]), 23)
