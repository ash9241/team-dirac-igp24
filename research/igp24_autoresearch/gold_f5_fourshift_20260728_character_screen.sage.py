...
"""Resolve and tc0-screen exact four-shift derivative-character hits."""
...
from sage.all import PolynomialRing, ZZ, prod


ROOT = Path(__file__).resolve().parent
DATA = ROOT / "data"
INPUT = DATA / "gold_f5_fourshift_20260728_character_scan.json"
RESULTS = DATA / "gold_f5_fourshift_20260728_character_screen.jsonl"
SUMMARY = DATA / "gold_f5_fourshift_20260728_character_screen_summary.json"
MANIFEST = ROOT / "outbox" / "gold_f5_fourshift_20260728_character_screen.txt"
RESERVED = {
    ("24T10482", 8),
    ("24T11787", 12),
    ("24T14293", 16),
...
TWO = load_module(
    "gold_f5_fourshift_two_screen",
    ROOT / "gold_f5_f6_20260728_two_shift_character_screen.sage.py",
)
SINGLE = TWO.SINGLE


def arithmetic_transform(quotient_line: str, shifts: tuple[int, ...]) -> dict:
    ...
    radicand = prod(u - ZZ(shift) for shift in shifts) * q.derivative()
    radicand %= q
    radicand_y = sum(
    ...
    source_line = SINGLE.coefficient_line(source_polynomial)
    transformed_line = SINGLE.coefficient_line(h)
    result = {
        "shifts": list(shifts),
        "elapsedSeconds": round(time.monotonic() - started, 3),
        "factorDegrees": [
        ...
        "sourcePolynomialSha256": SINGLE.sha_text(source_line),
        "transformedQuotientLine": transformed_line,
        "transformedQuotientSha256": SINGLE.sha_text(transformed_line),
    ...
    if len(degree_twelve) == 1:
        ...
        candidate_line = SINGLE.coefficient_line(candidate)
        result["candidate"] = {
            "coefficientLine": candidate_line,
            "coefficientSha256": SINGLE.sha_text(candidate_line),
...
def main() -> int:
    ...
    for hit in document["hits"]:
        source = hit["source"]
        shifts = tuple(int(value) for value in hit["shifts"])
        key = (str(source["coefficientSha256"]), shifts)
    ...
    for quotient_hash, cases in sorted(by_quotient.items()):
        ...
        cores = sorted(
            {core for case in cases for core in case["cores"]}
        )
        classification[quotient_hash] = (
            SINGLE.CHARACTER.character_alignment(
    ...
    for ordinal, (key, case) in enumerate(sorted(grouped.items()), 1):
        ...
        arithmetic = arithmetic_transform(
            str(source["coefficientLine"]), case["shifts"]
        )
        predictions = []
    ...
    summary = {
        "schemaVersion": "gold-f5-fourshift-20260728-character-screen-v1",
        ...
        "potentialPairs": [
            {"label": label, "r": r}
            for label, r in sorted(potential_pairs)
        ],
        "elapsedSeconds": round(time.monotonic() - started, 3),
