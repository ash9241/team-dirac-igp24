#!/usr/bin/env python3
"""GAP cycle atlas for the ordered-pair action of degree-12 groups."""
...
def build_gap_script() -> str:
    lines = [
        ...
        "SizeScreen([4096,]);",
        "CycleKey := function(p)",
        "  local lengths; lengths := SortedList(CycleLengths(p,[1..132]));",
        '  return JoinStringsWithSeparator(List(lengths,String),".");',
        "end;",
        "ordered := Filtered(Tuples([1..12],2), pair -> pair[1] <> pair[2]);;",
    ]
    for label in range(1, 302):
        lines += [
            f"g := TransitiveGroup(12,{label});; order := Size(g);;",
            "hom := ActionHomomorphism(g,ordered,OnTuples);;",
            "classes := ConjugacyClasses(g);;",
            "for cl in classes do",
            "  image := Image(hom,Representative(cl));;",
            f'  Print("ORDEREDINDEX|{label}|",CycleKey(image),"|",Size(cl),"|",order,"\\n");',
            "od;",
        ]
    lines.append("QUIT;")
    return "\n".join(lines) + "\n"
...
def parse_gap_output(output: str) -> dict[int, dict[str, float]]:
    ...
    for line in output.splitlines():
        if not line.startswith("ORDEREDINDEX|"):
            continue
    ...
    for label, patterns in counts.items():
        ...
        if total != orders[label]:
            raise ValueError(f"ordered atlas {label} sums to {total}, expected {orders[label]}")
        output_indices[label] = {pattern: count / total for pattern, count in patterns.items()}
...
def main() -> None:
    ...
    if len(payload) != 301:
        raise RuntimeError(f"GAP returned {len(payload)} ordered atlases, expected 301")
